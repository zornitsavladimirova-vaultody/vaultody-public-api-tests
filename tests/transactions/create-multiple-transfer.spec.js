...
// DESTRUCTIVE: creates a real transaction-request.
const RUN_DESTRUCTIVE = process.env.RUN_DESTRUCTIVE === 'true';
const hasData = Boolean(testData.testnet.toAddress);
...
test.describe('Transactions API - POST /vaults/{vaultId}/transaction-requests/multiple-transfer - positive', () => {

  test('should create a multiple-transfer transaction request', async ({ request }) => {
    test.skip(!canRunPositive, 'Set RUN_DESTRUCTIVE=true and testData.testnet.toAddress');
    await new Promise(resolve => setTimeout(resolve, 1000));
    const path = endpoints.transactions.multipleTransfer(testData.vaults.testVaultId);
    const payload = payloads.transactions.multipleTransfer({
      assetId: testData.assets.ethereum.assetId,
      recipients: [{ address: testData.testnet.toAddress, amount: '0.0001' }],
      note: 'pw-multiple-transfer',
      vaultAccountId,
...
test.describe('Transactions API - POST .../multiple-transfer - negative tests', () => {

  test('should return 400/409 when required fields are missing', async ({ request }) => {
    await new Promise(resolve => setTimeout(resolve, 1000));
    const path = endpoints.transactions.multipleTransfer(testData.vaults.testVaultId);
    const payload = payloads.transactions.multipleTransfer({ vaultAccountId }); // no assetId/recipients/note
    const headers = generateHeaders('POST', path, payload);
    const res = await request.post(`${CONFIG.baseUrl}${path}`, { headers, data: payload });
  ...
  test('should return 403 when vaultId does not belong to the API key', async ({ request }) => {
    await new Promise(resolve => setTimeout(resolve, 1000));
    const path = endpoints.transactions.multipleTransfer('000000000000000000000000');
    const payload = payloads.transactions.multipleTransfer({
      assetId: testData.assets.ethereum.assetId,
      recipients: [{ address: 'x', amount: '0.0001' }],
      note: 'n', vaultAccountId,
    });
    const headers = generateHeaders('POST', path, payload);
    const res = await request.post(`${CONFIG.baseUrl}${path}`, { headers, data: payload });
